...
from mvp.pyinvoice.components import SimpleTable, TableWithHeader, PaidStamp
from mvp.pyinvoice.models import PDFInfo, Item, Transaction, InvoiceInfo, ServiceProviderInfo, ClientInfo


class SimpleInvoice(SimpleDocTemplate):
    default_pdf_info = PDFInfo(title='Facture', author='damien.bernard@epitech.eu', subject='Facture')
    precision = None
    ...
    def _invoice_info_data(self):
        if isinstance(self.invoice_info, InvoiceInfo):
            props = [('invoice_id', 'N° facture'), ('invoice_datetime', 'Date de la facture'),
                     ('due_datetime', 'Date butoire')]
    ...
    def _service_provider_data(self):
        if isinstance(self.service_provider_info, ServiceProviderInfo):
            props = [('name', 'Nom'), ('street', 'Rue'), ('city', 'Ville'), ('state', 'State'),
                     ('country', 'Pays'), ('post_code', 'Code postal'), ('vat_tax_number', 'N° Siret')]
    ...
    def _item_data_and_style(self):
        ...
        item_data_title = ('Nom', 'Description', 'Unité', "Prix à l'unité", 'Montant')
